...
AI 驱动的 VMware vCenter/ESXi VM 生命周期管理与部署工具。监控、存储、Kubernetes 功能分别由配套技能提供。

> **VMware 技能家族（4 个配套技能）：**
>
> | 技能 | 命令 | 使用场景 |
> |------|------|---------|
> | **vmware-aiops** | `/vmware-ops:vmware-aiops` | VM 生命周期、部署、Guest Ops、Plan→Apply 编排 |
> | **vmware-monitor** | `/vmware-ops:vmware-monitor` | 只读监控：资源清单、健康检查、告警、事件、指标查询 |
> | **vmware-storage** | — | 数据存储、iSCSI、vSAN 管理 |
> | **vmware-vks** | — | Tanzu 命名空间、TKC 集群生命周期 |
...
> **已迁移至 [vmware-monitor](https://github.com/zw008/VMware-Monitor)**：列出虚拟机、主机、数据存储、集群、网络等资源清单功能现由 vmware-monitor 提供。

### 2. 健康监控

> **已迁移至 [vmware-monitor](https://github.com/zw008/VMware-Monitor)**：告警、事件日志、硬件传感器、主机服务状态等监控功能现由 vmware-monitor 提供。
...
# 虚拟机操作（资源清单和健康检查已迁移至 vmware-monitor）
...
| **[vmware-aiops](https://github.com/zw008/VMware-AIops)** | VM 生命周期、部署、Guest Ops | 31 | `uv tool install vmware-aiops` |
